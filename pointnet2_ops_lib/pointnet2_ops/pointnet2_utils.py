...
class DistNN(Function):
    ...
    def forward(ctx, unknown, known):
        ...
        idx : torch.Tensor
            (B, n, 3) index of 3 nearest neighbors
        """
        dist2 = _ext.dist_nn(unknown, known)
        # dist = torch.sqrt(dist2[0])

        # ctx.mark_non_differentiable(dist[0])
        ctx.mark_non_differentiable(dist2[0])
...
class PointSIFTSelect(Function):
    @staticmethod
    def forward(ctx, radius, xyz):
        # type: (Any, float, torch.Tensor) -> torch.Tensor
        r"""
        ...
        torch.Tensor
            (B, npoint, nsample) tensor with the indicies of the features that form the query balls
        """
        output = _ext.cube_select_sift(xyz, radius)
...
pointsift_select = PointSIFTSelect.apply
