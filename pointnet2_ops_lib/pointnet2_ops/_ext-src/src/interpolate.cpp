...
void dist_nn_kernel_wrapper(int b, int n, int m, const float *unknown,
...
std::vector<at::Tensor> dist_nn(at::Tensor unknowns, at::Tensor knows) {
  CHECK_CONTIGUOUS(unknowns);
  CHECK_CONTIGUOUS(knows);
  CHECK_IS_FLOAT(unknowns);
  CHECK_IS_FLOAT(knows);
  ...
  at::Tensor dist2 =
      torch::zeros({unknowns.size(0), unknowns.size(1), knows.size(1)},
                   at::device(unknowns.device()).dtype(at::ScalarType::Float));

  if (unknowns.is_cuda()) {
    dist_nn_kernel_wrapper(unknowns.size(0), unknowns.size(1), knows.size(1),
                            unknowns.data_ptr<float>(), knows.data_ptr<float>(),
